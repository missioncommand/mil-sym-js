...
armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsRenderer2={
    getMSRShapes : function (tg, shapes) {
        try {
            ...
            if (linetype != 25221000 && linetype != 25222000) return ;
            var hmap = armyc2.c2sd.JavaTacticalRenderer.clsUtility.getMSRSegmentColors (tg);
            var shape = null;
            var stroke = null;
            if (tg.get_Client ().equalsIgnoreCase ("ge")) stroke = armyc2.c2sd.JavaTacticalRenderer.clsUtility.getLineStroke2 (tg.get_LineThickness (), tg.get_LineStyle (), 1, 1);
            else stroke = armyc2.c2sd.JavaTacticalRenderer.clsUtility.getLineStroke (tg.get_LineThickness (), tg.get_LineStyle (), 1, 1);
            var j = 0;
            var color = null;
            var segShape = null;
    ...
    GetLineArray:function (tg, converter, isTextFlipped, clipBounds) {
        ...
        try {
            if (tg.Pixels == null || tg.Pixels.isEmpty ()) return null;
            var x = 0;
            var y = 0;
            var width = 0;
            var height = 0;
            ...
            switch (tg.get_LineType ()) {
                ...
                case 22612001:
                case 22623001:
                    if (tg.Pixels.size () < 2) return null;
                    if ((usas1314).booleanValue ()) break;
            ...
            if (bolResult) {
                ...
                if (tg.Pixels.size () < 2) if (lineType != 12000000) return null;
    ...
    getAutoshapeFillShape:function (tg, shapes) {
        try {
            ...
            switch (linetype) {
                case 211900000:
                    if (shapes != null && !shapes.isEmpty ()) for (j = 0; j < shapes.size (); j++) shapes.get (j).setFillColor (null);

                    shape.moveTo (tg.Pixels.get (0));
                    for (j = 1; j < 26; j++) shape.lineTo (tg.Pixels.get (j));
                ...
                case 211600000:
                    ...
                    shape.moveTo (tg.Pixels.get (0));
                    for (j = 1; j < tg.Pixels.size () - 3; j++) shape.lineTo (tg.Pixels.get (j));
                ...
                case 211400000:
                    ...
                    shape.moveTo (tg.Pixels.get (0));
                    for (j = 26; j < 47; j++) shape.lineTo (tg.Pixels.get (j));
