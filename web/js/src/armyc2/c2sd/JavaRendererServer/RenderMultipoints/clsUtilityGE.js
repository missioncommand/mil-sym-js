...
armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE={
    ...
    createDashedPolylines : function (tg, polylines, shape) {
        try {                
            ...
            var patternLength = 0;
            var numSegments = 0;
            for (j = 0; j < dash.length; j++) patternLength += dash[j];
            ...
            for (j = 0; j < polylines.size (); j++) 
            {
                ...
                for (k = 0; k < polyline.size () - 1; k++) 
                {
                    ...
                    if (noShortSegments) if (dist < 25) numSegments = 1;
    ...
    expandPolygon:function (pts, expand) {
        ...
        try {
            ...
            for (j = 0; j < pts2.size (); j++) poly.addPoint (Math.floor (pts2.get (j).x), Math.floor (pts2.get (j).y));
            ...
            for (j = 0; j < lineSegments.size (); j++) {
                ...
                m = (y1 - y2) / (x1 - x2);
                b = y1 - m * x1;
                if (j == lineSegments.size () - 1) nextLine = lineSegments.get (0);
                else nextLine = lineSegments.get (j + 1);
                x1 = nextLine.getX1 ();
                y1 = nextLine.getY1 ();
                x2 = nextLine.getX2 ();
                y2 = nextLine.getY2 ();
            ...
            for (j = 0; j < expandPts.size (); j++) lgPoly.add (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.setPoint2D (expandPts.get (j).x, expandPts.get (j).y));

            if (isClosed) lgPoly.add (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.setPoint2D (lgPoly.get (0).getX (), lgPoly.get (0).getY ()));
        } catch (exc) {
            if (Clazz.instanceOf (exc)) {
    ...
    expandPolygon2:function (pts, expandX, expandY) {
        ...
        try {
            ...
            if (isClosed) lgPoly.add (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.setPoint2D (destPts[0].getX (), destPts[0].getY ()));
        } catch (exc) {
            if (Clazz.instanceOf (exc)) {
    ...
    buildHatchFills2:function (tg, shapes) {
        try {
            ...
            var shape2 = null;
            var index = 0;
            if (armyc2.c2sd.JavaTacticalRenderer.clsUtility.isClosedPolygon (lineType) == false) if (armyc2.c2sd.JavaTacticalRenderer.clsUtility.IsChange1Area (lineType, null) == false) return ;
            switch (lineType) {
                case 24324100:
                case 24324300:
                case 24324200:
            ...
            for (var k = 0; k < shapes.size (); k++) {
                ...
                if (lineType != 243111000 && lineType != 243112000) break;
            }
            switch (lineType) {
                case 23115000:
                    for (j = 0; j < shapes.size (); j++) {
...
//                width /= 2;
//                for (j = 0; j < vertLimit; j++) {
//                    vertPt = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (x0 + width, y0 + 20 * j);
//                    vertPts.add (vertPt);
//                }
//                for (j = 0; j < horizLimit; j++) {
//                    horizPt = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (x0 + width - 20 * j, y0);
//                    horizPts.add (horizPt);
//                }
//                hatchLineShape.moveTo (armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (x0 + width + 10, y0 - 10));
//                hatchLineShape.lineTo (armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (x0, y0));
//                for (j = 0; j < vertLimit; j++) {
//                    if (j % 2 == 0) {
//                        hatchLineShape.lineTo (vertPts.get (j));
...
//                hatchLineShape.lineTo (armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (x0 + width + 10, y0 - 10));
    ...
    segmentColorsSet:function (tg) {
        try {
            ...
            var strH = tg.get_H ();
            if (strH == null || strH.isEmpty ()) return false;
            var strs = strH.$plit (",");
            if (strs.length > 1)
                return true;
        } catch (exc) {
...
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE._className= "clsUtilityGE";
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.Hatch_ForwardDiagonal= 2;
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.Hatch_BackwardDiagonal= 3;
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.Hatch_Vertical= 4;
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.Hatch_Horizontal= 5;
//armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.Hatch_Cross= 8;
