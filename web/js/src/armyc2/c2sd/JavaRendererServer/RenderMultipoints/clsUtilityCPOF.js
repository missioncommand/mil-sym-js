...
armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityCPOF={
    ...
    isValidShapeSpec :function (shape) {
        ...
        if (shape.getLineColor () == null && shape.getFillColor () == null) return false;
        if (shape.getShape () == null) return false;
        if (shape.getStroke () == null) return false;
        if (shape.getAffineTransform () == null) return false;
        return true;
    },
    SetLCColor:function (tg, shape) {
        try {
            ...
            if (affiliation.equals ("H")) {
                if (shape.getLineColor () === armyc2.c2sd.renderer.utilities.Color.RED) shape.setLineColor (tg.get_LineColor ());
                else shape.setLineColor (armyc2.c2sd.renderer.utilities.Color.RED);
            } else {
                if (shape.getLineColor () !== armyc2.c2sd.renderer.utilities.Color.RED) shape.setLineColor (tg.get_LineColor ());
                else shape.setLineColor (armyc2.c2sd.renderer.utilities.Color.RED);
            }
        ...
        try {
            ...
            if (originalShapes == null || originalShapes.length == 0) {
                ...
                for (j = 0; j < originalShapes.length; j++) {
                    ...
                    if (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityCPOF.isValidShapeSpec (shape) == true) newShapes.add (shape);
                }
                if (newShapes.size () > 0) {
    ...
    GetFarPixels:function (tg, converter, farLeftPixels, farRightPixels) {
        try {
            if (farLeftPixels == null || farRightPixels == null) return ;
            ...
            for (j = 0; j < tg.Pixels.size (); j++) {
                if (tg.Pixels.get (j).x < minX) minX = tg.Pixels.get (j).x;
                if (tg.Pixels.get (j).x > maxX) maxX = tg.Pixels.get (j).x;
            }
            var degWidth = (maxX - minX) * degLonPerPixel;
            if (Math.abs (degWidth) < 180) return ;
        ...
        try {
            ...
            switch (lineType) {
                ...
                case 243111000:
                    ...
                    if (tg.LatLongs.size () > 1) armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityCPOF.RangeFanOrientation (tg, lineType, converter);
                    break;
                case 243112000:
    ...
    Change1PixelsToShapes:function (tg, shapes) {
        ...
        for (k = 0; k < tg.Pixels.size (); k++) {
            ...
            if (k > 0) lastPt = tg.Pixels.get (k - 1);
            if (beginLine) {
                if (k == 0) shape.set_Style (currentPt.style);
                if (k > 0) if (currentPt.style == 5 && lastPt.style == 5) shape.lineTo (currentPt);
        ...
        try {
            ...
            for (l = 0; l < n; l++) {
                ...
                for (j = 0; j < pts.size (); j++) {
                    ...
                    ptTemp2.style = 0;
                    if (j == pts.size () - 1) ptTemp2.style = 5;
                    tg.Pixels.add (ptTemp2);
                }
            }
    ...
    GetSectorRangeFan :function (tg, converter) {
        ...
        try {
            ...
            for (k = 0; k < numSectors; k++) {
                ...
                pt1.style = 5;
                sectorPoints.add (pt1);
                for (l = 0; l < sectorPoints.size (); l++) allPoints.add (sectorPoints.get (l));
    ...
    RangeFanOrientation :function (tg, lineType, converter) {
        try {
            ...
            if (tg.LatLongs.size () > 1) {
                ...
                if (left > right) orientation = (left - 360 + right) / 2;
                else orientation = (left + right) / 2;
                dist = max;
            }
            radius = dist * 1.1;
    ...
    ClearPixelsStyle:function (tg) {
        try {
            ...
            for (var j = 0; j < tg.Pixels.size (); j++) tg.Pixels.get (j).style = 0;
                ...
                outer : while (removedPt == true) {
                    ...
                    for (j = 0; j < pts.size () - 1; j++) {
                        ...
                        if (dist < minSpikeDistance) {
                            if (segmented == false) {
                                if (j + 1 == pts.size () - 1) pts.remove (j);
                                else pts.remove (j + 1);
                                removedPt = true;
                                break outer;
    ...
    canClipPoints:function (tg) {
        try {
            ...
            if (armyc2.c2sd.JavaTacticalRenderer.clsMETOC.IsWeather (symbolId) > 0) return true;
            var linetype = tg.get_LineType ();
            //alert(linetype);
            switch (linetype) {
                case 2237000:
            ...
            case 25224000:
                ...
                if (shapes != null && shapes.size () > 0) {
                    for (var j = 0; j < shapes.size (); j++) {
                        shape = shapes.get (j);
                        if (shape.getShapeType () == armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE) shapes.get (j).setFillColor (null);
                    }
                }
                return true;
            default:
    ...
    Point2DtoPOINT2Mapped:function (pts2d, hashMap) {
        ...
        try {
            ...
            for (var j = 0; j < pts2d.size (); j++) 
            {
                //alert(j);
                pt2d = pts2d.get (j);
                if (hashMap.containsValue (pt2d)) style = 0;
                else style = -1;
    ...
    Point2DtoPOINT2:function (pts2d) {
        var pts =  new java.util.ArrayList ();
        try {
            for (var j = 0; j < pts2d.size (); j++) pts.add (armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pts2d.get (j).getX (), pts2d.get (j).getY ()));
    ...
    POINT2toPoint2D:function (pts) {
        ...
        try {
            for (var j = 0; j < pts.size (); j++) pts2d.add (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityGE.setPoint2D (pts.get (j).x, pts.get (j).y));
    ...
    buildShapeSpecFromPoints:function (shapeSpec, pts, clipArea) {
        ...
        try {
            ...
            if (clipBounds != null && clipBounds.containsRect (shapeSpec.getShape ().getBounds2D ()) == false && clipBounds.intersectsRect (shapeSpec.getShape ().getBounds2D ()) == false) {
                if (clipBounds.containsRect2 (x, y, w, h) == false && clipBounds.intersectsRect2 (x, y, w, h) == false) return shapeSpecs2;
            } else if (clipPoints != null) {
                var poly =  new armyc2.c2sd.graphics2d.GeneralPath ();
                for (j = 0; j < clipPoints.size (); j++) {
                    if (j == 0) poly.moveTo (clipPoints.get (j).getX (), clipPoints.get (j).getY ());
                    else poly.lineTo (clipPoints.get (j).getX (), clipPoints.get (j).getY ());
                }
                poly.closePath ();
                if (poly.containsRect (shapeSpec.getShape ().getBounds2D ()) == false && poly.intersectsRect (shapeSpec.getShape ().getBounds2D ()) == false) {
                    if (poly.containsRect2 (x, y, w, h) == false && poly.intersectsRect2 (x, y, w, h) == false) return shapeSpecs2;
                }
            }
            ...
            if (pts2.size () > 1) {
                ...
                tg.set_LineType (22124000);
                tg.Pixels = pts2;
                if (clipBounds != null) pts2d = armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsClipPolygon2.ClipPolygon (tg, clipBounds);
                else if (clipPoints != null) pts2d = armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsClipQuad.ClipPolygon (tg, clipPoints);
                if (pts2d != null && pts2d.size () > 1) 
                {
                    shape = armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtilityCPOF.BuildShapeFromPoints (pts2d);
                    gp.append (shape, false);
    ...
    segmentAnticipatedLine :function (tg) {
        try {
            var linetype = tg.get_LineType ();
            if (armyc2.c2sd.JavaTacticalRenderer.clsUtility.IsChange1Area (linetype, null)) return false;
            if (armyc2.c2sd.JavaRendererServer.RenderMultipoints.clsUtility.isAutoshape (tg)) return false;
            if (armyc2.c2sd.JavaTacticalRenderer.clsUtility.isBasicShape (linetype)) return false;
            switch (linetype) {
                case 23200000:
                case 23200001:
                case 22421000:
    ...
    postSegmentFSA:function (tg, converter) {
        try {
            if (tg.get_Client ().equals ("2D")) return ;
            var linetype = tg.get_LineType ();
            switch (linetype) {
                case 24326101:
                case 24321200:
            ...
            for (j = 0; j < latLongs.size () - 1; j++) {
                ...
                for (k = 1; k <= n; k++) {
                    ...
                    if (dist >= interval / 2) resultPts.add (pt);
                }
                resultPts.add (pt1);
            }
            latLongs = resultPts;
