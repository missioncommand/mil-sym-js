...
armyc2.c2sd.graphics2d.Rectangle2D=function(){
    ...
    this.containsPt2=function (x1, y1) {
        if (this.x <= x1 && x1 <= this.x + this.width && this.y <= y1 && y1 <= this.y + this.width) return true;
        else return false;
    };
    ...
    this.add=function (x1, y1) {
        ...
        if (x1 > this.x + this.width) this.width += (x1 - this.x);
        if (y1 > this.y + this.height) this.height += (y1 - this.y);
    };//, "~N,~N");
};
armyc2.c2sd.graphics2d.Rectangle2D.prototype.createIntersection=function (r) {
    if (r.x > this.x + this.width) return null;
    if (r.x + r.width < this.x) return null;
    if (r.y > this.y + this.height) return null;
    if (r.y + r.height < this.y) return null;
    if (r.containsRect (this)) return this;
    if (this.containsRect (r)) return r;
    var x1 = 0;
    var y1 = 0;
    var x2 = 0;
    var y2 = 0;
...
armyc2.c2sd.graphics2d.Rectangle2D.prototype.containsRect=function (rect) {
    ...
    if (this.containsPt2 (x1, y1)) {
        x1 += rect.getWidth ();
        y1 += rect.getHeight ();
        if (this.containsPt2 (x1, y1)) return true;
    }
    return false;
};
armyc2.c2sd.graphics2d.Rectangle2D.prototype.containsPt=function (pt) {
    if (this.x <= pt.getX () && pt.getX () <= this.x + this.width) if (this.y <= pt.getY () && pt.getY () <= this.x + this.height) return true;
    return false;
};
armyc2.c2sd.graphics2d.Rectangle2D.prototype.intersectsLine=function (line) {
    ...
    if (line.intersectsLine (edge)) return true;
