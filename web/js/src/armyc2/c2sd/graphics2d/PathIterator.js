...
armyc2.c2sd.graphics2d.PathIterator.prototype.isDone=function () {
    if (this._currentSeg == this._pts.size ()) return true;
    return false;
};
...
armyc2.c2sd.graphics2d.PathIterator.prototype.getBounds=function () {
    ...
    for (j = 1; j < this._pts.size (); j++) {
        if (this._pts.get (j).x < left) left = this._pts.get (j).x;
        if (this._pts.get (j).x > right) right = this._pts.get (j).x;
        if (this._pts.get (j).y < top) top = this._pts.get (j).y;
        if (this._pts.get (j).y > bottom) bottom = this._pts.get (j).y;
    }
