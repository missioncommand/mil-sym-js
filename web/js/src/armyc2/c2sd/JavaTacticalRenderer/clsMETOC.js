            ...
            switch (tg.get_LineType ()) {
                ...
                case 22623000:
                    ...
                    if (tg.get_FillColor () != null && tg.get_FillColor ().getAlpha () > 1) {
                        if (splinePoints != null && splinePoints.size () > 0) {
                            ...
                            for (j = 1; j < splinePoints.size (); j++) shape.lineTo (splinePoints.get (j));

                            shape.lineTo (tg.Pixels.get (tg.Pixels.size () - 1));
                            shapes.add (0, shape);
                        } else {
                            shape.moveTo (tg.Pixels.get (0));
                            for (j = 1; j < tg.Pixels.size (); j++) shape.lineTo (tg.Pixels.get (j));
                ...
                case 32163000:
                    ...
                    for (l = 0; l < partitions.size (); l++) {
                        ...
                        for (k = partitions.get (l).start; k <= partitions.get (l).end_Renamed + 1; k++) pixels.add (tg.Pixels.get (k));

                        if (pixels == null || pixels.isEmpty ()) continue ;
                ...
                case 32164000:
                    ...
                    for (l = 0; l < partitions.size (); l++) {
                        ...
                        for (j = 0; j < splinePointsArrays.size (); j++) {
                            ...
                            if (splinePoints2Arrays.size () > j) array2 = splinePoints2Arrays.get (j);
                            else break;
                            if (splinePointsArrays.size () >= splinePoints2Arrays.size ()) {
                                for (k = 0; k < array.size (); k++) {
                                    if (array.size () > k) pt = array.get (k);
                                    else break;
                ...
                case 32246000:
                    ...
                    if (splinePoints.size () > 0) lineObject2.moveTo (splinePoints.get (0).x, splinePoints.get (0).y);
                    else {
                        lineObject2.moveTo (tg.Pixels.get (0).x, tg.Pixels.get (0).y);
                        for (j = 0; j < tg.Pixels.size (); j++) lineObject2.lineTo (tg.Pixels.get (j).x, tg.Pixels.get (j).y);
                    ...
                    for (j = 1; j <= n; j++) {
                        if (splinePoints.size () >= j - 1) lineObject2.lineTo (splinePoints.get (j).x, splinePoints.get (j).y);
                    }
                    shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE);
                    shape.setShape (lineObject2);
                    shapes.add (shape);
        ...
        try {
            ...
            for (j = 0; j < shapes.size (); j++) {
                ...
                switch (tg.get_LineType ()) {
                    ...
                    case 22623001:
                    case 22612001:
                        if (shape.getShapeType () == armyc2.c2sd.renderer.utilities.ShapeInfo.SHAPE_TYPE_FILL) shape.setLineColor (null);
                        else {
                            shape.setLineColor (tg.get_LineColor ());
                            shape.set_Style (tg.get_LineStyle ());
                        }
        ...
        try {
            ...
            for (i = 0; i < array.size () - 1; i++) {
                ...
                if (tg.get_LineType () == 32164000 || tg.get_LineType () == 32164001) {
                    if (tmpArray.size () > 0) tmpArray.get (tmpArray.size () - 1).style = 47;
                }
                ...
                switch (tg.get_LineType ()) {
                    case 32272000:
                        if (i == array.size () - 2) {
                            if (splinePoints.size () >= 2) armyc2.c2sd.JavaTacticalRenderer.clsMETOC.DrawArrow (splinePoints.get (splinePoints.size () - 2), tg.Pixels.get (tg.Pixels.size () - 1), 10, lineObject);
                        }
                        break;
                    case 32273000:
                        if (i == array.size () - 2) {
                    ...
                    case 32273001:
                        if (i == array.size () - 2) {
                            ...
                            for (j = 1; j < splinePoints2.size (); j++) lineObject.lineTo (Math.floor (splinePoints2.get (j).x), Math.floor (splinePoints2.get (j).y));
        ...
        try {
            ...
            switch (tg.get_LineType ()) {
                ...
                case 22623000:
                    lineObject.moveTo (P0.x, P0.y);
                    lineObject.curveTo (P1.x, P1.y, P2.x, P2.y, P3.x, P3.y);
                    if (tg.get_FillColor () == null || tg.get_FillColor ().getAlpha () < 2) return array;
                    break;
                case 32163001:
                case 32610001:
                case 32234301:
