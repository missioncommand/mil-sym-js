...
armyc2.c2sd.JavaTacticalRenderer.clsUtility={
    ...
    linesOutsideClipBounds:function (tg, clipBounds) {
        try {
            ...
            for (j = 0; j < tg.Pixels.size () - 1; j++) {
                ...
                if (clipBounds.containsPt2 (pt0.x, pt0.y)) return false;
                if (clipBounds.containsPt2 (pt1.x, pt1.y)) return false;
                ...
                if (ptsLine.intersectsLine (boundsEdge)) return false;
    ...
    tweakFillColor:function (tg) {
        try {
            if ((armyc2.c2sd.JavaTacticalRenderer.clsUtility.isSameColor (tg.get_LineColor (), tg.get_FillColor ())).booleanValue () === false) return ;
    ...
    isSameColor:function (c1, c2) {
        try {
            ...
            var b1 = c1.getBlue ();
            var b2 = c2.getBlue ();
            if (Math.abs (r1 - r2) < 5) if (Math.abs (g1 - g2) < 5) if (Math.abs (b1 - b2) < 5) return true;
        } catch (exc) {
            if (Clazz.instanceOf (exc)) {
    ...
    GetSectorRadiiFromPoints:function (tg) {
        try {
            if (tg.get_LineType () == 243111001) return ;
                    ...
                    if (tg.get_Affiliation () != null && tg.get_Affiliation ().equals ("H")) {
                        switch (quadrant) {
                            ...
                            case 4:
                                ...
                                for (var j = 0; j < tg.Pixels.size (); j++) tg.Pixels.set (j, pts2.get (n - j - 1));
                        ...
                        switch (quadrant) {
                            ...
                            case 3:
                                ...
                                for (j = 0; j < tg.Pixels.size (); j++) tg.Pixels.set (j, pts2.get (n - j - 1));
    ...
    initializeLinetypes:function (rev) {
        try {
            if (armyc2.c2sd.JavaTacticalRenderer.clsUtility.linetypes != null || armyc2.c2sd.JavaTacticalRenderer.clsUtility.metocs != null) return ;
    ...
    setRevC:function (tg) {
        try {
            ...
            if (rev == 1) {
                switch (tg.get_LineType ()) {
                    case 212210000:
                        if (tg.Pixels.size () == 4) tg.set_LineType (212210001);
                        break;
                    case 212220000:
                        if (tg.Pixels.size () == 4) tg.set_LineType (212220001);
                        break;
                    case 212230000:
                        if (tg.Pixels.size () == 4) tg.set_LineType (212230001);
                        break;
                    case 212300000:
                        if (tg.Pixels.size () == 4) tg.set_LineType (212300001);
                        break;
                    case 24326100:
            ...
            switch (tg.get_LineType ()) {
                case 212210001:
                    if (tg.Pixels.size () < 4) tg.set_LineType (212210000);
                    break;
                case 212220001:
                    if (tg.Pixels.size () < 4) tg.set_LineType (212220000);
                    break;
                case 212230001:
                    if (tg.Pixels.size () < 4) tg.set_LineType (212230000);
                    break;
                case 212300001:
                    if (tg.Pixels.size () < 4) tg.set_LineType (212300000);
                    break;
                default:
                    break;
            }
    ...
    getMSRSegmentColors:function (tg) {
        ...
        try {
            ...
            switch (linetype) {
                ...
                case 25222000:
                case 22121000:
                    if (tg.get_H () == null || tg.get_H ().isEmpty ()) return null;
    ...
    reviseHModifier:function (originalPixels, tg) {
        try {
            if (tg.get_H () == null || tg.get_H ().isEmpty ()) return ;
            var linetype = tg.get_LineType ();
            switch (linetype) {
                case 25222000:
                case 25221000:
            ...
            for (var key, $key = hmap.keySet ().iterator (); $key.hasNext () && ((key = $key.next ()) || true);) {
                if ((key).intValue () < n - 1) continue ;
                if ((key).intValue () + 1 > originalPixels.size () - 1) break;
                color = hmap.get (key);
                ...
                for (j = 0; j < tg.Pixels.size () - 1; j++) {
                    ...
                    if (segPt0.x == pt0.x && segPt0.y == pt0.y) {
                        ...
                        if (pt0.x == segPt1.x && pt0.y == segPt1.y) continue ;
                        if (pt1.x == segPt0.x && pt1.y == segPt0.y) continue ;
                        else {
    ...
    InterpolatePixels:function (tg) {
        try {
            if (tg.get_UseLineInterpolation () == false) return ;
            var linetype = tg.get_LineType ();
            var glyphSize = 10;
            switch (linetype) {
                case 23131100:
            ...
            for (j = 0; j < tg.Pixels.size (); j++) {
                if (hmapPixels.containsKey (new Integer (j))) pixels.add (hmapPixels.get (new Integer (j)));
                if (hmapGeo.containsKey (new Integer (j))) geo.add (hmapGeo.get (new Integer (j)));
            }
            switch (linetype) {
                case 22350000:
                case 22134000:
