...
armyc2.c2sd.JavaLineArray.arraysupport=
{
    ...
    GetLineArray2: function (lineType, pts, shapes, clipBounds, rev) {
        ...
        try {
            ...
            if (vblCounter > 0) pLinePoints =  new Array (vblCounter);
            else {
                shapes = null;
                return null;
            }
            armyc2.c2sd.JavaLineArray.lineutility.InitializePOINT2Array (pLinePoints);
            if (vblSaveCounter > pts.size ()) vblSaveCounter = pts.size ();
            if (vblSaveCounter > pLinePoints.length) vblSaveCounter = pLinePoints.length;
            for (j = 0; j < vblSaveCounter; j++) {
                pt = pts.get (j);
                pLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pt.x, pt.y, pt.style);
            }
        ...
        try {
            ...
            if (Math.abs (m0.value[0]) >= 1 || bolVertical == 0) {
                ...
                for (j = 0; j < vblCounter - 1; j++) {
                    if (index != j) {
                        if (lineType == 23111001 && oppSegment != j) continue ;
        ...
        try {
            ...
            for (j = 0; j < vblSaveCounter - 1; j++) {
                ...
                for (k = 0; k < n; k++) {
                    ...
                    nCounter++;
                    if (lineType == 22624000) pSpikePoints[nCounter++] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pSpikePoints[nCounter - 4]);
                }
                pSpikePoints[nCounter++] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[j + 1]);
            }
            for (j = 0; j < nCounter; j++) {
            ...
            if (lineType == 23115000) {
                ...
                if (nCounter > 0) pSpikePoints[nCounter - 1].style = 5;
            }
            for (j = 0; j < nCounter; j++) {
                pLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pSpikePoints[j]);
                if (j == nCounter - 1) {
    ...
    GetIsolatePointsDouble : function (pLinePoints, lineType) {
        try {
            ...
            if (pt0.x == pt1.x && pt0.y == pt1.y) pt1.x += 1;
    ...
    GetReefPoints : function (pLinePoints, vblCounter) {
        ...
        try {
            ...
            for (var j = 0; j < vblCounter - 1; j++) {
                if (pOriginalPoints[j].x < pOriginalPoints[j + 1].x) direction = 2;
                else direction = 3;
                d = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble (pOriginalPoints[j], pOriginalPoints[j + 1]);
    ...
    GetRestrictedAreaPoints : function (pLinePoints, vblCounter) {
        ...
        try {
            ...
            for (var j = 0; j < vblCounter - 1; j++) {
                ...
                if (pOriginalPoints[j].x < pOriginalPoints[j + 1].x) direction = 3;
                else direction = 2;
                for (var k = 0; k < numSegs; k++) {
    ...
    getOverheadWire : function (pLinePoints, vblCounter) {
        ...
        try {
            ...
            for (j = counter; j < pLinePoints.length; j++) pLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[counter - 1]);
    ...
    GetLineArray2Double : function (lineType, pLinePoints, vblCounter, vblSaveCounter, shapes, clipBounds, rev) {
        ...
        try {
            ...
            if (pLinePoints == null || pLinePoints.length < 2) return null;
            var segments = null;
            var dMRR = 0;
            var n = 0;
            var bolVertical = 0;
            ...
            var lFlotCount = 0;
            if (vblCounter > 2) pt2 = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[2]);
            pt0.style = 0;
            pt1.style = 0;
            pt2.style = 0;
            var xPoints = null;
            ...
            switch (lineType) {
                ...
                case 32153000:
                    ...
                    break;
                case 32152000:
                    if (pLinePoints[0].x < pLinePoints[1].x) armyc2.c2sd.JavaLineArray.lineutility.ReversePointsDouble2 (pLinePoints, vblSaveCounter);
                ...
                case 31134000:
                    ...
                    for (j = 0; j < vblSaveCounter; j++) pLinePoints[vblCounter + j] = pOriginalLinePoints[j];
                ...
                case 31132100:
                    ...
                    for (j = 0; j < vblSaveCounter; j++) pLinePoints[vblCounter - vblSaveCounter + j] = pOriginalLinePoints[j];
                ...
                case 2237000:
                    ...
                    break;
                case 22431100:
                    for (j = 0; j < vblCounter; j++) pLinePoints[j].style = 1;
                ...
                case 23111001:
                    ...
                    for (j = 0; j < vblSaveCounter; j++) pLowerLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[j]);

                    for (j = 0; j < vblSaveCounter; j++) pUpperLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[j]);
                    ...
                    for (j = 0; j < vblSaveCounter; j++) pUpperLowerLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pUpperLinePoints[j]);

                    for (j = 0; j < vblSaveCounter; j++) pUpperLowerLinePoints[j + vblSaveCounter] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLowerLinePoints[vblSaveCounter - j - 1]);
                    ...
                    for (j = 0; j < vblCounter; j++) pLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pUpperLowerLinePoints[j]);
                ...
                case 22221000:
                    ...
                    acCounter = 0;
                    for (j = 0; j < vblSaveCounter; j++) if (pOriginalLinePoints[j].style <= 0) pOriginalLinePoints[j].style = 1;
                ...
                case 22330000:
                    ...
                    if (d < 20) pLinePoints[1] = armyc2.c2sd.JavaLineArray.lineutility.ExtendLineDouble (pLinePoints[0], pLinePoints[1], 21);
                ...
                case 23225000:
                    ...
                    if (clipBounds != null) {
                        ...
                        if (dist0 > dist1) armyc2.c2sd.JavaLineArray.lineutility.LineRelativeToLine (pLinePoints[0], pLinePoints[1], pt0, pt4, pt5);
                        else armyc2.c2sd.JavaLineArray.lineutility.LineRelativeToLine (pLinePoints[0], pLinePoints[1], pt1, pt4, pt5);
                        pLinePoints[2] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pt5);
                        pLinePoints[3] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pt4);
                    }
                    ...
                    pointCounter++;
                    if ((drawJaggies).booleanValue ()) while (dExtendLength < dWidth - 10) {
                        dExtendLength = n * 5;
                ...
                case 22522210:
                    ...
                    if (dMBR < 500) dMBR = 500;
                    if (dMBR > 750) dMBR = 500;
                    d = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble (pLinePoints[0], pLinePoints[1]);
                    if (d < dMBR / 40) pLinePoints[1] = armyc2.c2sd.JavaLineArray.lineutility.ExtendLineDouble (pLinePoints[0], pLinePoints[1], dMBR / 40 + 1);
                ...
                case 22522100:
                    ...
                    if (d > 60) {
                        ...
                        if (middleSegment >= 1) pt3 = pLinePoints[middleSegment - 1];
                    } else {
                        if (vblSaveCounter <= 3) middleSegment = 1;
                    ...
                    if (vblSaveCounter == 2) pLinePoints[1].style = 5;
                    for (j = 0; j < vblCounter; j++) airPts.add (armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLinePoints[j]));
                    ...
                    for (j = 0; j < airPts.size (); j++) pLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (airPts.get (j));
                ...
                case 211200000:
                    d = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble (pLinePoints[0], pLinePoints[1]);
                    if (client.startsWith ("cpof")) d2 = 20;
                    else d2 = 30;
                    d2=30;
                    if (d < d2) {
                        lineType = 22522220;
                    ...
                    break;
                case 211210000:
                    if (client.startsWith ("cpof")) d2 = 25;
                    else d2 = 25;
                    var folspDist = 0;
                    ...
                    //if (dMBR > 250) dMBR = 250;
                    if (client.startsWith ("cpof")) {
                        if (folspDist < 25) dMBR = 125;
                        if (folspDist < 75) dMBR = 150;
                        if (folspDist < 100) dMBR = 175;
                        if (folspDist < 125) dMBR = 200;
                    } 
                    else 
                    {
                        dMBR *= 1.5;
                ...
                case 23223000:
                    lLinestyle = 9;
                    if (dMBR / 10 > armyc2.c2sd.JavaLineArray.arraysupport.maxLength) dMBR = 10 * armyc2.c2sd.JavaLineArray.arraysupport.maxLength;
                    ...
                    for (k = 0; k < 3; k++) pLinePoints[vblCounter - 6 + k] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pArrowPoints[k]);
                    ...
                    for (k = 0; k < 3; k++) pLinePoints[vblCounter - 3 + k] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pArrowPoints[k]);
            ...
            switch (lineType) {
                ...
                case 32214000:
                    ...
                    for (k = 0; k < vblSaveCounter; k++) {
                        ...
                        if (k == 0) whiteShape.moveTo (pLinePoints[k]);
                        else whiteShape.lineTo (pLinePoints[k]);
                    }
                ...
                case 32550000:
                    ...
                    for (k = 1; k < vblSaveCounter; k++) redShape.lineTo (pLinePoints[k]);
                    ...
                    for (k = vblSaveCounter; k < acCounter; k++) {
                        if (pLinePoints[k].style == 0) {
                            if (beginLine) {
                                ...
                                beginLine = false;
                            } else blueShape.lineTo (pLinePoints[k]);
                        }
                ...
                case 31134200:
                    ...
                    for (k = 0; k < acCounter; k++) {
                        if (pLinePoints[k].style == 22) {
                            ...
                            if (redShape != null && redShape.getShape () != null) shapes.add (redShape);
                        }
                        if (pLinePoints[k].style == 20) {
                            ...
                            if (blueShape != null && blueShape.getShape () != null) shapes.add (blueShape);
                        }
                    }
                    break;
                case 31134100:
                ...
                case 31134000:
                    ...
                    for (k = 1; k < vblSaveCounter; k++) blackShape.lineTo (pOriginalLinePoints[k]);
                ...
                case 31132200:
                    ...
                    for (k = 0; k < acCounter; k++) {
                        if (pLinePoints[k].style == 20) {
                            ...
                            if (shape != null && shape.getShape () != null) shapes.add (shape);
                        }
                    }
                    break;
                case 211200000:
                    ...
                    for (j = 2; j < vblCounter; j++) {
                        if (pLinePoints[j - 1].style != 5) shape.lineTo (pLinePoints[j]);
                        else shape.moveTo (pLinePoints[j]);
                    }
                    shapes.add (shape);
                    break;
                case 31131200:
                ...
                case 22330000:
                    for (k = 0; k < vblCounter; k++) {
                        ...
                        if (shape == null) shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE);
                        if (beginLine) {
                            if (k > 0) if (pLinePoints[k].style == 5 && pLinePoints[k - 1].style == 5) continue ;
                ...
                case 23225000:
                    ...
                    for (k = 5; k < acCounter; k++) {
                        if (pLinePoints[k - 1].style != 5) shape.lineTo (pLinePoints[k]);
                    }
                    if (shape != null && shape.getShape () != null) shapes.add (shape);
                    break;
                case 22350000:
                    ...
                    for (k = vblCounter; k < points.size (); k++) {
                        if (beginLine) {
                            if (k == 0) shape.set_Style (points.get (k).style);
                            if (k > 0) if (points.get (k).style == 5 && points.get (k - 1).style == 5) shape.lineTo (points.get (k));
                ...
                case 221311000:
                    shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE);
                    shape.moveTo (pLinePoints[0]);
                    for (k = 1; k < acCounter - 5; k++) shape.lineTo (pLinePoints[k]);
                ...
                case -1:
                    shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE);
                    shape.moveTo (pLinePoints[0]);
                    for (k = 1; k < pLinePoints.length; k++) shape.lineTo (pLinePoints[k]);
                ...
                default:
                    for (k = 0; k < acCounter; k++) {
                        ...
                        if (beginLine) {
                            ...
                            if (k > 0) {
                                if (pLinePoints[k].style == 5 && pLinePoints[k - 1].style == 5 && k < acCounter - 1) continue ;
                                else if (pLinePoints[k].style == 5 && pLinePoints[k - 1].style == 10) continue ;
                            }
                            if (k == 0 && pLinePoints.length > 1) if (pLinePoints[k].style == 5 && pLinePoints[k + 1].style == 5) continue ;
                            shape.moveTo (pLinePoints[k]);
                            beginLine = false;
                        } else {
                            shape.lineTo (pLinePoints[k]);
