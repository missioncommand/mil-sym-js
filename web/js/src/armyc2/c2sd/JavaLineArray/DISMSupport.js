...
armyc2.c2sd.JavaLineArray.DISMSupport=
{
    ...
    GetTGFontSize  : function (iLength) {
        var result = -1;
        try {
            if (iLength < 20) result = 0;
            else if (iLength < 50) result = 1;
            else if (iLength > 250) result = 3;
            else result = 2;
        } catch (exc) {
    ...
    ArcApproximationDouble : function (left, top, right, bottom, startx, starty, endx, endy, lpoints) {
        try {
            ...
            if (y1 == 0) {
                ...
                else startAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI;
            } else if (x1 == 0) {
                if (y1 > 0) startAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI * 0.5;
                else startAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI * -0.5;
            } else startAngle = Math.atan2 (y1, x1);
            if (y2 == 0) {
                if (x2 > 0) endAngle = 0;
                else endAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI;
            } else if (x2 == 0) {
                if (y2 > 0) endAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI * 0.5;
                else endAngle = armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI * -0.5;
            } else endAngle = Math.atan2 (y2, x2);
            if (endAngle <= startAngle) endAngle += 2 * armyc2.c2sd.JavaLineArray.DISMSupport.CONST_PI;
    ...
    GetDISMCoverDouble: function (points, linetype) {
        ...
        try {
            ...
            if (pt1.x < pt2.x && quadrant == 1) sign = -1;
            else if (pt1.x > pt2.x && quadrant == 2) sign = -1;
            else if (pt1.x > pt2.x && quadrant == 3) sign = -1;
            else if (pt1.x < pt2.x && quadrant == 4) sign = -1;
            if (linetype == 22139000) t = 0;
            if (points[1].x <= points[2].x) goLeftThenRight = true;
            for (j = 0; j < 3; j++) {
                savepoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (points[j]);
            ...
            if (iFontSize > 0) {
                ...
                if (goLeftThenRight) savepoints[0].x -= 30 * t;
                else savepoints[0].x += 30 * t;
                iLetterOffset = 0;
                ...
                if (goLeftThenRight) savepoints[0].x += 60 * t;
                else savepoints[0].x -= 60 * t;
                ptsJaggyLine[0].x = savepoints[0].x + iLetterOffset * 2;
                ptsJaggyLine[0].y = savepoints[0].y;
                ptsJaggyLine[0].x += iLetterOffset;
                ...
                for (j = 0; j < 3; j++) {
                    points[counter] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (ptsJaggyLine[j]);
                    points[counter].style = 0;
                    if (linetype == 22139000) points[counter].style = 9;
                    counter++;
                }
                points[counter - 1].style = 5;
                if (linetype == 22139000) {
    ...
    GetDISMCoverDoubleRevC: function (points, linetype, vblSaveCounter) {
        ...
        try {
            ...
            if (vblSaveCounter == 4) {
                ...
                for (j = 0; j < vblSaveCounter; j++) origPoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (points[j]);
            ...
            if (iFontSize > 0) {
                ...
                if (vblSaveCounter < 4) ptsJaggyLine[1] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (savepoints[1]);
                else ptsJaggyLine[1] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (origPoints[0]);
                if (vblSaveCounter < 4) {
                    ptsJaggyLine[2].x = savepoints[1].x + dDeltaX1;
                    ptsJaggyLine[2].y = savepoints[1].y + dDeltaY1;
                } else {
    ...
    GetDISMFixDouble: function (points, linetype, clipBounds) {
        ...
        try {
            ...
            points[counter].style = 5;
            counter++;
            if ((drawJaggies).booleanValue ()) while (i <= iNumJaggies) {
    ...
    GetDISMSeizeDouble: function (points, linetype, radius) {
        ...
        try {
            ...
            var iDiagEOL_length = 0;
            var factor = 1;
            if (radius > 0) iCircleRadius = radius;
            for (j = 0; j < 3; j++) {
                savepoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (points[j]);
            }
            ...
            if (!client.startsWith ("cpof") && radius == 0) {
                ...
                if (iCircleRadius > dArcRadius / 2) iCircleRadius = dArcRadius / 2;
            }
            armyc2.c2sd.JavaLineArray.lineutility.InitializePOINT2Array (pts);
            armyc2.c2sd.JavaLineArray.lineutility.InitializePOINT2Array (arcpoints);
    ...
    GetDISMRIPDouble: function (points, linetype) {
        ...
        try {
            ...
            if (side01 == armyc2.c2sd.JavaLineArray.DISMSupport.RIGHT_SIDE && side12 == armyc2.c2sd.JavaLineArray.DISMSupport.RIGHT_SIDE) clockwise = true;
            else if (side01 == armyc2.c2sd.JavaLineArray.DISMSupport.RIGHT_SIDE && side12 == armyc2.c2sd.JavaLineArray.DISMSupport.COLINEAR) clockwise = true;
            else if (side01 == armyc2.c2sd.JavaLineArray.DISMSupport.COLINEAR && side12 == armyc2.c2sd.JavaLineArray.DISMSupport.RIGHT_SIDE) clockwise = true;
            for (j = 0; j < 4; j++) {
                savepoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (points[j]);
            }
            armyc2.c2sd.JavaLineArray.lineutility.InitializePOINT2Array (pts);
    ...
    AmbushPointsDouble: function (pLinePoints) {
        ...
        try {
            ...
            if (iArrowLength > armyc2.c2sd.JavaLineArray.DISMSupport.maxLength) iArrowLength = Math.floor (armyc2.c2sd.JavaLineArray.DISMSupport.maxLength);
            if (iArrowLength < armyc2.c2sd.JavaLineArray.DISMSupport.minLength) iArrowLength = Math.floor (armyc2.c2sd.JavaLineArray.DISMSupport.minLength);
            ...
            pLinePoints[counter - 1].style = 5;
            d = dRadius / 3;
            if (d > armyc2.c2sd.JavaLineArray.DISMSupport.maxLength) d = armyc2.c2sd.JavaLineArray.DISMSupport.maxLength;
            if (d < armyc2.c2sd.JavaLineArray.DISMSupport.minLength) d = armyc2.c2sd.JavaLineArray.DISMSupport.minLength;
