...
{    
    ...
    CalcSegmentAngleDouble: function (pt0, pt1) {
        ...
        try {
            ...
            if (nTemp == 0) dAngle = 1.5707963267948966;
            else {
    ...
    CalcTrueSlopeDouble: function (firstLinePoint, lastLinePoint, slope) {
        var result = 1;
        try {
            if (slope.value == null) slope.value =  Clazz.newArray (1, 0);
            var deltaX = 0;
            var deltaY = 0;
    ...
    ReversePointsDouble2: function (pLowerLinePoints, vblCounter) {
        try {
            ...
            var k = 0;
            for (k = 0; k < vblCounter; k++) pResultPoints[k] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pLowerLinePoints[vblCounter - k - 1]);

            for (k = 0; k < vblCounter; k++) pLowerLinePoints[k] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pResultPoints[k]);
    ...
    GetOffsetPointDouble: function (startLinePoint, endLinePoint, nOffset) {
        ...
        try {
            ...
            else dAngle = Math.atan (dx / dy) + 1.5707963267948966;
            dHypotenuse = nOffset;
    ...
    LineOfXPoints: function (pLinePoints) {
        ...
        try {
            ...
            for (j = 0; j < pLinePoints.length - 1; j++) {
                ...
                if (dist - iterations * 20 > 10) iterations += 1;
...
//    },
    ...
    CalcDirectionFromLine: function (pt0, pt1, pt2) {
        ...
        try {
            ...
            var ptIntersect = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 ();
            if (pt0.x == pt1.x) {
                if (pt2.x < pt0.x) return 0;
                else return 1;
            }if (pt0.y == pt1.y) {
                if (pt2.y < pt0.y) return 2;
                else return 3;
            ...
            if (m1.value[0] > 1) {
                if (pt2.x < ptIntersect.x) return 0;
                else return 1;
            } else {
                if (pt2.y < ptIntersect.y) return 2;
                else return 3;
            }} catch (e) {
...
//    },
    ...
    GetArrowHead4Double: function (startLinePoint, endLinePoint, nBiSector, nBase, pResultLinePoints, styl) {
        try {
            ...
            var tempLinePoint = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (startLinePoint);
            if (dy == 0) {
                if (dx > 0) dAngle = 3.141592653589793;
                else dAngle = 0;
            } else dAngle = Math.atan (dx / dy) + 1.5707963267948966;
            tempLinePoint.style = 0;
            if (dx <= 0.0 && dy <= 0.0) dSign = -1.0;
            if (dx >= 0.0 && dy <= 0.0) dSign = -1.0;
            if (dx <= 0.0 && dy >= 0.0) dSign = 1.0;
            if (dx >= 0.0 && dy >= 0.0) dSign = 1.0;
    ...
    CalcTrueIntersectDouble: function (m1, b1, m2, b2, p2, bolVerticalSlope1, bolVerticalSlope2, dWidth, lOrient, X, Y) {
        try {
            ...
            X.value =  Clazz.newArray (1, 0);
            Y.value =  Clazz.newArray (1, 0);
            if (m1 != m2 && Math.abs (m1 - m2) <= 4.9E-324) m1 = m2;
            if (b1 != b2 && Math.abs (b1 - b2) <= 4.9E-324) b1 = b2;
    ...
    GetSAAFRSegment: function (pLinePoints, lineType, dMRR, rev) {
        try {
            ...
            if (lineType == 22223000) pLinePoints[0].style = 5;
            if (rev == 1) pLinePoints[0].style = 5;
        } catch (exc) {
            if (Clazz.instanceOf (exc)) {
    ...
    ArcArrayDouble: function (pResultLinePoints, vblCounter, dRadius, linetype) {
        try {
            ...
            if (bolVertical != 0) M = Math.atan (m.value[0]);
                ...
                if (a.y < e.y) M = -1.5707963267948966;
                else M = 1.5707963267948966;
            ...
            switch (linetype) {
                ...
                case 23173000:
                case 231100000:
                    for (j = 0; j < numarcpts; j++) pResultLinePoints[j] = armyc2.c2sd.JavaLineArray.lineutility.setPOINT2 (pArcLinePoints[j]);
    ...
    CalcCircleShape: function (Center, radius, numpts, CirclePoints, styl) {
        var shape;
        if (styl == 9) shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_FILL);
        else shape =  new armyc2.c2sd.JavaLineArray.Shape2 (armyc2.c2sd.JavaLineArray.Shape2.SHAPE_TYPE_POLYLINE);
    ...
    GetDitchSpikeDouble: function (pLinePoints, nOldCounter, bWayIs, linetype) {
        ...
        try {
            ...
            for (i = 0; i < nOldCounter - 1; i++) {
                if (linetype == 23132000 && i == 0) minLength = 38;
                ...
                if (length > minLength * dPrinter) {
                    if (bWayIs != 0) {
                        ...
                        }if (j % 2 == 1 && linetype == 23132000) pLinePoints[nSpikeCounter].style = 5;
                        nSpikeCounter++;
    ...
    RotateGeometryDouble: function (pLinePoints, vblCounter, lAngle) {
        try {
            ...
            if (lAngle != 0) {
                ...
                for (j = 0; j < vblCounter; j++) {
                    if (pLinePoints[j].x == pdCenter.x) {
                        if ((pLinePoints[j].y > pdCenter.y)) dGamma = 4.71238898038469;
                        else dGamma = 1.5707963267948966;
                    } else dGamma = 3.141592653589793 + Math.atan ((pLinePoints[j].y - pdCenter.y) / (pLinePoints[j].x - pdCenter.x));
                    if (pLinePoints[j].x >= pdCenter.x) dGamma = dGamma + 3.141592653589793;
                    dTheta = dRotate + dGamma;
                    y = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble (pLinePoints[j], pdCenter) * Math.sin (dTheta);
                    x = armyc2.c2sd.JavaLineArray.lineutility.CalcDistanceDouble (pLinePoints[j], pdCenter) * Math.cos (dTheta);
                    pLinePoints[j].y = pdCenter.y + y;
...
//    setPOINT2: function () {
//        var pt =  new armyc2.c2sd.JavaLineArray.POINT2 ();
//        return pt;
//    },//);
//    setPOINT2: function (pt) {
//        var pt2 =  new armyc2.c2sd.JavaLineArray.POINT2 ();
//        pt2.x = pt.x;
//        pt2.y = pt.y;
//        pt2.segment = pt.segment;
//        pt2.style = pt.style;
//        return pt2;
//    },//, "armyc2.c2sd.JavaLineArray.POINT2");
...
//        return pt2;
//    },//, "~N,~N");
//    setPOINT2: function (x, y, segment, style) {
//        var pt2 =  new armyc2.c2sd.JavaLineArray.POINT2 ();
//        pt2.x = x;
//        pt2.y = y;
//        pt2.segment = segment;
...
//        return pt2;
//    },//, "~N,~N,~N,~N");
//    //c$.setPOINT2 = Clazz.defineMethod (c$, "setPOINT2", 
//    setPOINT2: function (x, y, style) {
//        var pt2 =  new armyc2.c2sd.JavaLineArray.POINT2 ();
//        pt2.x = x;
//        pt2.y = y;
//        pt2.style = style;
